...
//		labelCount = 0;
	...
	public void storeStatement(AbstractSyntaxTree node) {
		...
		while (!queue.isEmpty()) {
			...
			case SCAN: this.addStatement(new TACScanStatement(n.getNodeType(), (Variable) this.storeExpression(n.getChild(0)))); flag = false; break;
			default:
				break;
			}
...
//		SymbolContext ctx = new SymbolContext(lNode.getLiteralType(), this.currentScope, lNode.getValue().toString());
		...
		if(n.getValue() != null) {
			...
			TACAssignStatement aStmt = new TACAssignStatement(NodeType.ASSIGN, OPERATOR.getEnum(n.getValue()), new Variable(OperandTypes.VARIABLE, value, ctx.getIdentifier()), value);
...
//		this.addStatement(new TACGotoStatement(ICGenerator.LABEL_ALIAS+(this.labelCount-1)));
	...
	private Operand storeExpression(AbstractSyntaxTree node) {
		...
		while(!queue.isEmpty()) {
			...
			System.out.println(aStmt.getValue().toString()+"------------------------------------------------------");
